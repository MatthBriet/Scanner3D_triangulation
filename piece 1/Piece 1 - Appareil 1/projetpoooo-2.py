...
class Ligne(Piece):
    ...
    def changement_repere_camera(self):
        Xcamera=[]
        Ycamera=[]
        for i in range(len(self.ligne)):
            Ycamera.append((-self.ligne[i][0]+4000/2)/k)
            Xcamera.append((self.ligne[i][1]-3089)/k)
        self.Camera.append(Xcamera)
        self.Camera.append(Ycamera)
        plt.plot(Xcamera,Ycamera)
        plt.show()
    
    def changement_repere_monde(self,f,beta,alpha): #alpha = angle d'inclinaison de la camera; beta=angle entre le laser et la camera (alpha= 20,b=30 f=52) a mettre en radian
        Xm=[]
        Ym=[]
        Zm=[]
...
b.affichage()
b.ligne_laser()
b.changement_repere_camera()
b.changement_repere_monde(52,30,20)
b.transfo_array()

plt.plot(b.Monde[1],b.Monde[2])
plt.show()
print(b.Monde)
#print(b.ligne)
print('a')
...
def bidule(ligne):
    ...
    ligne.SizeImage()
    ligne.matrice_seuil()
    ligne.subpixel_matrice()
    #b.affichage()
    ligne.ligne_laser()
    ligne.changement_repere_camera()
    ligne.changement_repere_monde(52,30,20)
    ligne.passage_3D()
    ligne.affichage_ligne()
...
#    k = (1290/197)*(1430/52)
#    for i in  range (len(image)):          #les pixels comprenant la piece pour pas faires des calculs long sur des étapes innutiles...
...
#    
#    return [Xm,Ym,Zm]
#plt.plot(changement_repere_monde(changement_repere_camera('01.jpg'),52,0.523,0.349)[1],changement_repere_monde(changement_repere_camera('01.jpg'),52,0.523,0.349)[2],".")   
